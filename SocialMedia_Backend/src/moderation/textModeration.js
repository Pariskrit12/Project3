...
export async function moderateText(text) {
  ...
  try {
    const flagged = leoProfanity.check(text.trim());
    return { flagged, reason: flagged ? "profanity" : null };
  } catch (err) {
    console.error("[textModeration] bad-words check failed, allowing content through:", err.message);
    return { flagged: false, reason: null };
  }
}

// --- OpenAI moderation (disabled, restore when API key is active) ---
